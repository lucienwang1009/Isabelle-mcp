...
__all__ = ["allowed_roots", "read_theory_file", "resolve_in_sandbox"]
...
def read_theory_file(path: str) -> str:
    """Read a ``.thy`` file's text from within the sandbox.

    Raises ``invalid_argument`` for non-``.thy`` or oversized files, and
    ``file_not_found`` for missing files or paths outside the allow-list.
    """
